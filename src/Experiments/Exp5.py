...
from Utils.ExperimentUtils import deltas_magnitude
    ...
    for proj, paths in data.iteritems():
        ...
        if not proj in paths.bellw:
            ...
            for train_bellw, validation in CrossValidation.split(bellw,
                ...
                res[proj[:6]]["alves"].append(deltas_magnitude(orig, patched_alves))
                res[proj[:6]]["olive"].append(deltas_magnitude(orig, patched_olive))
                res[proj[:6]]["shatw"].append(deltas_magnitude(orig, patched_shatw))
    ...
    for res in changes(data):
        for key, value in res.iteritems():
            ...
            for n, (attr, xtree_local, xtree_bellw, Olive, Alves, Shatw) in \
                ...
                print(n, attr[1:], xtree_local, xtree_bellw, Olive, Alves, Shatw, sep="\t")
